...
@Component({
  selector: 'app-creditmoney',
  templateUrl: './creditmoney.component.html',
  styleUrls: ['./creditmoney.component.css']
})
export class CreditmoneyComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.getRetailerCommission('BAL_DR');
    } 
 // function to get difference between from date and to date 
  ...
  exportAsXLSX(): any {
    const excelData: any = [];
    const header = ['Trn Date', 'Txn Id','User Name', 'Narration', 'Amount(CR)','Amount(DR)','Charge', 'Commission','TDS','Balance'];
    ...
    this.excelService.exportAsExcelWitheHeader(header,excelData, 'LedgerReport.xlsx','I1');
  }
  returnNarration(cp: any): any {
    if (cp.DocType==='OBAL') {
      ...
      return 'Account Debited To#'+ cp.TxnNo + ' ,'+ cp.ToNarration;
    }
