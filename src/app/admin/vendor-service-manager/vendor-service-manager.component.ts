...
@Component({
  selector: 'app-vendor-service-manager',
  templateUrl: './vendor-service-manager.component.html',
  styleUrls: ['./vendor-service-manager.component.css']
})
export class VendorServiceManagerComponent implements OnInit {
  ...
  GetMemberType(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.getAuth('servicemanager/get-service-manager?mid=&docType=VDR')
        .subscribe(
          (res: any) => {
            console.log(res);
            if (res.isSuccess) {
