...
@Component({
  selector: 'app-commission-kyc',
  templateUrl: './commission-kyc.component.html',
  styleUrls: ['./commission-kyc.component.css']
})
export class CommissionKycComponent implements OnInit {
  ...
  public GetMemberType(uid: any) {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      ...
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + uid + '&docType=KYC&isstutas=' + this.isstutas)
        .subscribe(
          (res: any) => {
  ...
  openForm(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.isLoader = true;
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + this.uuid + '&docType=KYC&isstutas=NEW')
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess) {
