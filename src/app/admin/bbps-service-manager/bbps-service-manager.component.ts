...
@Component({
  selector: 'app-bbps-service-manager',
  templateUrl: './bbps-service-manager.component.html',
  styleUrls: ['./bbps-service-manager.component.css']
})
export class BbpsServiceManagerComponent implements OnInit {
  mbType: any = {};
  userList: any;
  respData: any;
  walletList: any;
  ...
  GetMemberType(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.getAuth('servicemanager/get-service-manager?mid=&docType=BBPS')
        .subscribe(
          (res: any) => {
            console.log(res);
            if (res.isSuccess) {
