...
@Component({
  selector: 'app-commission-vendor',
  templateUrl: './commission-vendor.component.html',
  styleUrls: ['./commission-vendor.component.css']
})
export class CommissionVendorComponent implements OnInit {
  mbType: any = {};
  userList: any;
  ...
  public GetMemberType(uid: any) {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      ...
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + uid + '&docType=VDR&isstutas=' + this.isstutas)
        .subscribe(
          (res: any) => {
  ...
  openForm(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.isLoader = true;
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + this.uuid + '&docType=VDR&isstutas=NEW')
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess) {
