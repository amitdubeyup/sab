...
export class AepsTransactionComponent implements OnInit {
  ...
  getRetailerCommission(docType: any): void {
    ...
    if (this.commonService.userPram.userId && this.commonService.userPram.memberId) {
      ...
      this.commonService.getAuth('aepsapi/get-aepstransaction?memberId=' + this.selectedMemberList?.code +  this.retid?.code + '&fromDt=' + fromDt + '&toDt=' + toDt + '&isStatus=' + this.isStatus + '&docType=' + docType+'&st='+this.searchType+'&txnNo='+this.txnNo)
        .subscribe(
          (res: any) => {
  ...
  onsaveAdmin(){
       ...
       this.commonService.postAES256('aepsapi/aepsPayNow', JSON.stringify(this.serviceData)).subscribe((res: any) => {
        this.commonService.isLoader=false; 
        if (res.isSuccess) {
