...
@Component({
  selector: 'app-commission-bbps',
  templateUrl: './commission-bbps.component.html',
  styleUrls: ['./commission-bbps.component.css']
})
export class CommissionBbpsComponent implements OnInit {
  mbType: any = {};
  userList: any;
  ...
  openForm(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      ...
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + this.uuid + '&docType=BBPS&isstutas=NEW')
        .subscribe(
          (res: any) => {
  ...
  public GetMemberType(uid: any) {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.isLoader = true;
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + uid + '&docType=BBPS&isstutas=' + this.isstutas)
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess) {
