...
@Component({
  selector: 'app-commission-aeps',
  templateUrl: './commission-aeps.component.html',
  styleUrls: ['./commission-aeps.component.css']
})
export class CommissionAepsComponent implements OnInit {
  mbType: any = {};
  userList: any;
  ...
  openForm(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      ...
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + this.uuid + '&docType=AEPS&isstutas=NEW')
        .subscribe(
          (res: any) => {
  ...
  public GetMemberType(uid: any) {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.isLoader = true;
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + uid + '&docType=AEPS&isstutas=' + this.isstutas)
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess) {
