...
export class StatementComponent implements OnInit {
  ...
  getRetailerCommission(docType:any): void {
    ...
    if (this.commonService.userPram.userId && this.commonService.userPram.memberId) {
      this.commonService.getAuth('retailerreport/get-transaction-report-admin?memberId=' + this.selectedMemberList + '&fromDt=' + fromDt + '&toDt=' + toDt + '&isStatus='+this.isStatus+ '&docType='+docType+'&st='+this.searchType+'&txnNo='+this.txnNo)
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess) {
   ...
   payNow(): any {
    ...
    getTrasExport(docType:any): void {
      ...
      if (this.commonService.userPram.userId && this.commonService.userPram.memberId) {
        ...
        if(this.activeTab === 7)
        {
          uri='membersapi/get-fund-approved-report?memberId=' + this.selectedMemberList + '&fromDt=' + fromDt + '&toDt=' + toDt + '&isStatus='+this.isStatus+ '&docType='+docType;
        }
        else if (this.activeTab === 8)
        {
          ...
          uri='retailerreport/get-transaction-report-export?memberId=' + this.selectedMemberList + '&fromDt=' + fromDt + '&toDt=' + toDt + '&isStatus='+this.isStatus+ '&docType='+docType +'&st='+this.searchType+'&txnNo='+this.txnNo;
        }
          ...
          .subscribe(
            (res: any) => {
              ...
              if (res.isSuccess) {
               ...
               statementListExport.forEach((el: any) => {
                ...
                else
                {
                  const obj = [
                    ...
                    el.remName,
                    el.narration +','+el.beneficiary_acc+','+el.udf1,
                    el.amount,
                    el.amountDr,
                    el.charge,   
                    el.rt_commission,            
               ...
               let header = ['Txn#','Txn Date', 'Updated','Code', 'User Name','Sender','Beneficiary','Amount(CR)','Amount(DR)','Charge','Commission','Product','Mode','API','Status','UTR','Refrence No','Refund Txn No','Responce'];
               if(this.activeTab ===5)
               {
                 header = ['Txn#','Txn Date', 'Updated On', 'SPR Code','Send By','Recived By','Amount(CR)','Amount(DR)','Charge','Service','PayMode','API','Status','UTR','TnxRefNo','RtxnNo','Resp.'];
               }
               this.excelService.exportAsExcelWitheHeader(header,excelData, 'TransactionReport.xlsx','K1');
              }
            },
            (err: any) => {
              this.commonService.isLoader = false;
