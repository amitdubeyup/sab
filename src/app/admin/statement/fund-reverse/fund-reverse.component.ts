...
@Component({
  selector: 'app-fund-reverse',
  templateUrl: './fund-reverse.component.html',
  styleUrls: ['./fund-reverse.component.css']
})
export class FundReverseComponent implements OnInit {
  respData: any;
  fromDt: any = new Date();
  toDt: any = new Date();
  statementList: any = [];
   ...
   payNow(): any {
    ...
    returnNarrationAll(cp: any): any {
      if (cp.service==='OBAL') {
        ...
        return  'Money Transfer :  A/C#' + cp.beneficiary_acc + ', Mob#'+cp.beneficiary_number +', Ben#' + cp.narration +',IFSC#'+cp.toNarration ;
      }
