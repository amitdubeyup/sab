...
export class BalanceManagerComponent implements OnInit {
  ...
  WalletDetails(): any {
    const url = `users/getprantuser?memberId=${this.commonService.userPram.memberId}&userId=${this.commonService.userPram.userId}&parentId=${this.commonService.userPram.parentMemberId}&docType=admin`;
  ...
  getMember(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.getAuth('users/GetMemberById?memberId=' + this.commonService.userPram.memberId + '&roleType=' + this.commonService.userPram.memberType + '&docType=SA')
        .subscribe(
          (res: any) => {
            if (res.isSuccess) {
  ...
  fetchCreditHistory(): void {
   ...
   {
    this.commonService.getAuth('distributor/getdistributorbal?memberId=' + this.coutletid + '&dimemberid=' + this.commonService.userPram.memberId + '&txnNo=' + this.txnNo + '&fromDt=' + fromDt + '&toDt=' + toDt + '&docType=BAL_DR_ADMIN').subscribe((res: any) => {
      console.log(res);
      this.commonService.isLoader = false;
      if (res.isSuccess) {
  ...
  onCreditMoneySave(): any {
    ...
    {
    const walletBalance = this.pramwallet?.walleT_BALANCE;
    if (this.creditMoneyModal.amount < walletBalance) {
      ...
      const passingData = {
        ...
        udf2: this.commonService.userPram.companyName+' | '+this.commonService.userPram.userType+''+this.commonService.userPram.id,
        Ifsc: '',
        intent: this.creditMoneyModal.remark,
        docType: 'BAL_CR',
        recipientId: '',
        accNo: '',
        BeneName: this.commonService.userPram.companyName+' | '+this.commonService.userPram.userType+''+this.commonService.userPram.id,
      ...
      this.commonService.postAES256('wallets/managefundadmin', JSON.stringify(passingData))
        .subscribe(
          (res: any) => {
            console.log(res);
            this.commonService.isLoader = false;
            Swal.fire({ icon: 'success', text: res.mhOutcome, confirmButtonText: 'OK' }).then(() => {
              ...
              setTimeout(() => {
                Object.keys(this.creditMoneyForm.controls).forEach(key => {
                  ...
                  this.selectedMemberCr.code=null;
                });
              },600);
              this.fetchCreditHistory();
            });
  ...
  exportAsXLSX(): any {
    let url='';
    let exproDataList:any;
    this.commonService.isLoader = true;
    const fromDt = this.commonService.formatDate((this.fromDt ? this.fromDt : new Date()), 'yyyy-mm-dd');
    const toDt = this.commonService.formatDate((this.toDt ? this.toDt : new Date()), 'yyyy-mm-dd');
    url='distributor/getdistributortransactionbal?memberId=' + this.coutletid + '&dimemberid=' + this.commonService.userPram.memberId + '&txnNo=' + this.txnNo + '&fromDt=' + fromDt + '&toDt=' + toDt + '&docType=BAL_DR_ADMIN'
    this.commonService.getAuth(url).subscribe((res: any) => {
      console.log(res);
      this.commonService.isLoader = false;
      if (res.isSuccess) {
        exproDataList = JSON.parse(this.commonService.decryptUsingAES256(res.respData));
        const excelData: any = [];
        exproDataList.forEach((el: any) => {
      const obj = [
         el.CreatedOn,
         el.TxnNo,
         el.Narration,
      ...
      const header = ['Tran Date','Txn','Outlet','Amount','Narration'];
      this.excelService.exportAsExcelWitheHeader(header,excelData, 'Balance-Manager-Report.xlsx','E1');
    }
    }, (err: any) => {
      console.log(err);
      this.commonService.isLoader = false;
