...
@Component({
  selector: 'app-pg-manager',
  templateUrl: './pg-manager.component.html',
  styleUrls: ['./pg-manager.component.css']
})
export class PgManagerComponent implements OnInit {
  ...
  getMembers(): any {
    if (this.commonService.userPram.userId !== undefined && this.commonService.userPram.userId !== '' && this.commonService.userPram.rmn !== undefined && this.commonService.userPram.rmn !== '' && this.commonService.userPram.memberLevel !== undefined && this.commonService.userPram.memberLevel !== '') {
      ...
      this.commonService.getAuth('servicemanager/get-admin-bank-list?memberid=' + this.commonService.userPram.memberId +'&docType=ADPG')
        .subscribe(
          (res: any) => {
