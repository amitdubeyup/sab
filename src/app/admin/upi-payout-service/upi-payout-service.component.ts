...
@Component({
  selector: 'app-upi-payout-service',
  templateUrl: './upi-payout-service.component.html',
  styleUrls: ['./upi-payout-service.component.css']
})
export class UpiPayoutServiceComponent implements OnInit {
  ...
  GetMemberType(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.getAuth('servicemanager/get-service-manager?mid=&docType=UPI-1')
        .subscribe(
          (res: any) => {
            console.log(res);
            if (res.isSuccess) {
  ...
  addService(): any {
    ...
    this.serviceData.apiType = 'UPI-1';
