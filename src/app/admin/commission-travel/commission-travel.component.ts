...
@Component({
  selector: 'app-commission-travel',
  templateUrl: './commission-travel.component.html',
  styleUrls: ['./commission-travel.component.css']
})
export class CommissionTravelComponent implements OnInit {
  ...
  public GetMemberType(uid: any) {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      ...
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + uid + '&docType=TRVL&isstutas=' + this.isstutas)
        .subscribe(
          (res: any) => {
  ...
  openForm(): any {
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.isLoader = true;
      this.commonService.getAuth('servicemanager/get-commission-template-det?uuid=' + this.uuid + '&docType=TRVL&isstutas=NEW')
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess) {
