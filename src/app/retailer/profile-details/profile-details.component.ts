...
export class ProfileDetailsComponent implements OnInit {
  ...
  fetchWithdrawHistory(): void {
    ...
    this.commonService.getAuth('users/profile?userid=' + this.uuid + '&docType=ID').subscribe((res: any) => {
      console.log(res);
      this.commonService.isLoader = false;
      if (res.isSuccess) {
        this.profileData = JSON.parse(this.commonService.decryptUsingAES256(res.respData));
        console.log( this.profileData);
        if (this.profileData.comm) {
          this.profileData.comm.forEach((element: any) => {
