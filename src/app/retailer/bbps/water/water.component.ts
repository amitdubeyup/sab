...
export class WaterComponent implements OnInit {
          ...
          (res: any) => {
            ...
            if (res.isSuccess && res.respData) {
              ...
              if (res.respData.apiId === '08d93b98-21fc-49c9-8130-9ad5f53a3c9a') {
                this.getoprators('WATER','icore');
              } else if (res.respData.apiId === '8b53435a-faae-11eb-9029-f80dac58cdd1') {
                this.getoprators('WATER','levin');
              }
              else
              {
                this.getoprators('WATER','mobik');
              }
            }
          },
          (err: any) => {
  ...
  getPaymentPipe(maxAmount: any): any {
    this.commonService.isLoader = true;
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService.getAuth('recharge/get-api-service?isMaxAmt=true&ApiType=BBPS&Category=WATER&maxAmount=' + maxAmount)
        .subscribe(
          (res: any) => {
            console.log(res);
            this.commonService.isLoader = false;
  ...
  payNow(): any {
    if (this.pipeList && this.pipeList.apiId) {
      ...
      if (paymentAmount < walletBalance) {
        console.log(this.utilityModal.mPin, savedPin);
  ...
  getRecentTransaction(): void {
    ...
    if (this.commonService.userPram.userId && this.commonService.userPram.memberId) {
      this.commonService.isLoader = true;
      this.commonService.getAuth('retailerreport/get-recent-transaction?memberId=' + this.commonService.userPram.memberId + '&fromDt=' + fromDt + '&toDt=' + toDt + '&docType=WATER')
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess) {
