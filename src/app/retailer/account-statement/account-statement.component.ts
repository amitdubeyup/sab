 ...
 // function to get difference between from date and to date 
  ...
  returnNarration(cp: any): any {
    if (cp.DocType==='OBAL') {
      ...
      return 'Account Credited By#'+ cp.TxnNo + ' ,'+ cp.ToNarration;
    }
    else if (cp.DocType=='BAL_DR') {
      return 'Account Debited By#'+cp.TxnNo+' ,'+ cp.ToNarration;
    }
    ...
    else if (cp.DocType=='AEPS' && cp.ModeOfPayment=='AEPS-CW') {
      return 'Aeps Commission Credited #'+ cp.TxnNo + ' , Ref#' + (cp.ReferenceNo==null?'':cp.ReferenceNo);
    }
