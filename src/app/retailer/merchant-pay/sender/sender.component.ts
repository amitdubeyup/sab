...
export class SenderComponent implements OnInit {
        ...
        if (res.isSuccess) {
          ...
          Swal.fire({ icon: 'success', text: res.mhOutcome, confirmButtonText: 'Okay!' }).then((result) => {
            if (result.isConfirmed) {
              this.router.navigate(['/merchant/merchant-transfer/beneficiary'], { queryParams: { remitterId: res.remitterId, paymentMode: this.paymentMode} });
            }
          });
         }
        }
  ...
  onBeneficiary(data: any): any {
    this.router.navigate(['/merchant/merchant-transfer/beneficiary'], { queryParams: { remitterId: data.RemitterId, paymentMode: this.paymentMode,acc:'' } });
  }
  onBeneficiaryView(data: any): any {
    this.router.navigate(['/merchant/merchant-transfer/beneficiary'], { queryParams: { remitterId: data.RemitterId, paymentMode: this.paymentMode,acc:data.BeneficiaryAcc } });
  }
