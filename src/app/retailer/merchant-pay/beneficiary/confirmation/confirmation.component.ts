          ...
          .subscribe(
            (res: any) => {
              ...
              Swal.fire({ icon: 'success', text: res.mhOutcome, confirmButtonText: 'OK' }).then(() => {
                ...
                this.commonService.fetchWalletDetails();
                this.router.navigate(['/rt/merchant-transfer/beneficiary/receipt'], { queryParams: {remitterId:this.remitterDetails.RemitterId,paymentMode:this.paymentMode,data: jsonData } });
              });
              }
              else
              {
