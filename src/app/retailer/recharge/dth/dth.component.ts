...
export class DthComponent implements OnInit {
  ...
  toggle(): any {
    this.displayDTH = !this.displayDTH;
  }
  getoprators(docType: any, apiType: any): any {
    if (
      this.commonService.userPram.userId !== undefined &&
      this.commonService.userPram.userId !== '' &&
      this.commonService.userPram.rmn !== undefined &&
      this.commonService.userPram.rmn !== ''
    ) {
      this.commonService.isLoader = true;
      this.commonService
        .getAuth(
          'commonapi/get-oprators?docType=' + docType + '&apiType=' + apiType
        )
        .subscribe(
          (res: any) => {
  ...
  getOperatorPipe(): any {
    ...
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService
        .getAuth(
          'recharge/get-api-service?isMaxAmt=false&ApiType=BBPS&Category=BILLFETCH&maxAmount=0'
        )
        .subscribe(
          (res: any) => {
            this.commonService.isLoader = false;
            if (res.isSuccess && res.respData) {
              ...
              if (
                res.respData.apiId === '08d93b98-21fc-49c9-8130-9ad5f53a3c9a'
              ) {
                this.getoprators('GAS', 'icore');
              } else if (
                res.respData.apiId === '8b53435a-faae-11eb-9029-f80dac58cdd1'
              ) {
  ...
  getPaymentPipe(maxAmount: any): any {
    ...
    if (this.commonService.userPram.userId && this.commonService.userPram.rmn) {
      this.commonService
        .getAuth(
          'recharge/get-api-service?isMaxAmt=true&ApiType=BBPS&Category=GAS&maxAmount=' +
            maxAmount
        )
        .subscribe(
          (res: any) => {
  ...
  fetchBiller(): any {
    ...
    this.commonService.isLoader = true;
    this.commonService
      .postAES256(`recharge/get-fetch-bill`, JSON.stringify(passingData))
      .subscribe(
        (res: any) => {
          console.log(res);
          this.commonService.isLoader = false;
  ...
  payNow(): any {
    if (this.pipeList && this.pipeList.apiId) {
      ...
      const paymentAmount = this.activeTab
        ? this.utilityModal.paymentAmount
        : this.billerDetails?.billamount;
      if (paymentAmount < walletBalance) {
        if (this.utilityModal.mPin === savedPin) {
          ...
          this.commonService.isLoader = true;
          this.commonService
            .postAES256('recharge/bill-pay', JSON.stringify(passingData))
            .subscribe(
              (res: any) => {
                console.log(res);
                this.commonService.isLoader = false;
                Swal.fire({
                  ...
                  if (res.isSuccess) {
                    this.router.navigate(['/retailer/bill-payment/receipt'], {
  ...
  getRecentTransaction(): void {
    ...
    if (
      ...
      this.commonService
        .getAuth(
          'retailerreport/get-recent-transaction?memberId=' +
            this.commonService.userPram.memberId +
            '&fromDt=' +
            fromDt +
            '&toDt=' +
          ...
          (res: any) => {
            ...
            if (res.isSuccess) {
              this.billList = JSON.parse(
                this.commonService.decryptUsingAES256(res.respData)
              );
