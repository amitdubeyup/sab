...
@Component({
  selector: 'app-distributor-statement',
  templateUrl: './distributor-statement.component.html',
  styleUrls: ['./distributor-statement.component.css']
})
export class DistributorStatementComponent {
respData: any;
fromDt: any = new Date();
toDt: any = new Date();
statementList: any = [];
...
else
{
  if (this.commonService.userPram.userId && this.commonService.userPram.memberId) {
    ...
    this.commonService.getAuth('retailerreport/get-transaction-commission?memberId=' + this.dist_id + '&fromDt=' + fromDt + '&toDt=' + toDt + '&docType=' + docType)
      .subscribe(
        (res: any) => {
